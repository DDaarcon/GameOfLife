...
CellNeighbours checkNeighbours(const Stage& stage_, const sf::Vector2i& coord_) {
	...
	if (stage_.checkIfXIn(coord_.x) &&
		stage_.checkIfYIn(coord_.y)){
		...
		if (coord_.x > 1){
			...
			// . . .
			// x . .
			if (stage_.checkIfYIn(coord_.y + 1)){
				if (stage_.getCellState(sf::Vector2i(coord_.x - 1, coord_.y + 1)) == Cell::States::Alive){
					cellNeighbours.alive++;
				} else {
					cellNeighbours.dead++;
		...
		// . . x
		// . . x
		if (stage_.checkIfYIn(coord_.x + 1)){
			// . . x
			// . . .
			// . . .
			if (coord_.y > 1){
...
void updateStageCells(Stage& stage_) {
	...
	Stage tempStage(Cell::States::Dead, sf::Vector2i(stage_.getX(), stage_.getY()));
	
	for (int x = 1; stage_.checkIfXIn(x); x++)
		for (int y = 1; stage_.checkIfYIn(y); y++){
	...
	for (int x = 1; stage_.checkIfXIn(x); x++)
		for (int y = 1; stage_.checkIfYIn(y); y++){
			if (stage_.getCellState(sf::Vector2i(x, y)) != tempStage.getCellState(sf::Vector2i(x, y)))
				stage_.paintCell(sf::Vector2i(x, y), tempStage.getCellState(sf::Vector2i(x, y)));
		}
